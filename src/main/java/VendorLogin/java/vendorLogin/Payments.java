...
public void Payments1() throws Throwable {
		    driver.findElement(By.xpath("//div[contains(text(),'Payment Summary')]")).click();
			Thread.sleep(2000);
			TakesScreenshot js = (TakesScreenshot)driver;
			File source = js.getScreenshotAs(OutputType.FILE);
			File target = new File("./screenshots/Bookings/001PaymentSummaryDetails.png");
			FileUtils.copyFile(source, target); 
}
@Test(priority = 2, enabled = true)
public void Payments2() throws Throwable {
			driver.findElement(By.xpath("//div[contains(text(),'Payment History')]")).click();
			...
			TakesScreenshot js = (TakesScreenshot)driver;
			File source = js.getScreenshotAs(OutputType.FILE);
			File target = new File("./screenshots/Bookings/002PaymentHistoryList.png");
			FileUtils.copyFile(source, target); 
}
